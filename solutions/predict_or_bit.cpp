...
struct BlockOr
{
	...
	explicit BlockOr(int _mask) : mask(_mask) {}

	void set_bit(int p, bool val)
	{
		if (!val)
			mask &= ~(1U << p);
		else
	...
	BlockOr apply_reversed(BlockOr o, int a, int b, int len)
	{
		int omask = bit_rev[len][(o.mask >> b) & ((1 << len) - 1)];
		return BlockOr(mask | (omask << a));
	}
};
...
struct DPVector
{
	...
	{
		v[p / S].set_bit(p % S, val);
	}
	bool get(int p)
	{
		return v[p / S].get_bit(p % S);
	}
	BlockOr &operator[](int p)
	{
		return v[p];
	}
	void extend(bool val)
	{
		while ((int)v.size() * S < _size)
			v.push_back(BlockOr(val ? -1 : 0));
	}
	bool push_back(bool x)
	{
		_size++;
		extend(false);
		bool old_x = get(_size - 1);
		set(_size - 1, x);
		return old_x;
	}
	void pop_back(bool old_x = false)
	{
		set(--_size, old_x);
	}
	int size()
	...
	void resize(int new_len, bool val)
	{
		_size = new_len;
		extend(val);
	}
...
{
	HE_UNDEF, HE_SET, HE_PUSH_BACK, HE_SET_VEC, HE_SET_BLOCK_VEC, HE_SWAP_VEC, HE_RESIZE_VEC
};
...
struct HistoryManager
{
	...
	{
		events.push_back(HistoricEvent(HE_SET_VEC, ptr, id, ptr->get(id)));
		ptr->set(id, val);
	}

	void set_block_vec(DPVector *ptr, int id, BlockOr val)
	{
		events.push_back(HistoricEvent(HE_SET_BLOCK_VEC, ptr, id, (*ptr)[id].mask));
		(*ptr)[id] = val;
	}
	...
	void undo()
	{
		while (!events.empty())
		{
			...
			if (ev.type == HE_PUSH_BACK)
				...
				ptr->set(ev.arg1, ev.arg2);
			else if (ev.type == HE_SET_BLOCK_VEC)
				(*ptr)[ev.arg1] = BlockOr(ev.arg2);
...
void DPVector::apply_seg(int l, int r, DPVector &from, int c, int d, bool rev, bool, bool history)
{
	if (!rev)
	{
		for (int i = l; i <= r; i++)
	...
	else
	{
		for (int i = c; i <= d; )
		{
			...
			int len2 = min(j - l + 1, j % S + 1);
			int len = min(len1, len2);
			if (history)
				hist.set_block_vec(this, p2, v[p2].apply_reversed(from[p1], j % S - len + 1, i % S, len));
			else
				v[p2] = v[p2].apply_reversed(from[p1], j % S - len + 1, i % S, len);
			i += len;
		}
	}
}
...
void calc_dp(int v, int i)
{
	...
	per_dp[per].apply_seg(idx, idx, old_dp, i - baby_len + 1, i - baby_len + 1, false);
...
void predict_dp(int v, int start, int until)
{
	...
	if (left <= right)
	{
		dp.apply_seg(left + 1, right + 1, old_dp,
...
void predict_small_pals(int i, int until)
{
	...
	dp.apply_seg(i + 1, i + cur_r, old_dp, i - cur_r + 1, i, true, true, true);
	cur_r = get_rad(cent(i) - 1, until - 1);
	dp.apply_seg(i + 1, i + cur_r, old_dp, i - cur_r, i - 1, true, true, true);
}
