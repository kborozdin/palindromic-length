...
void add_char(int i)
{
	...
	//to_check.resize(unique(to_check.begin(), to_check.end()) - to_check.begin());
...
void predict_dp(int v, int start, int until)
{
	...
	dp.apply_seg(start + 1, start + full, sub, 0, sub.size() - 1, false, false, false);
	...
	if (left <= right)
	{
		dp.apply_seg(left + 1, right + 1, dp,
			rpos(reflect(cent(right), big_center)), rpos(reflect(cent(left), big_center)), true, true, false);
	}
}
...
void predict_small_pals(int i, int until)
{
	...
	dp.apply_seg(i + 1, i + cur_r, dp, i - cur_r + 1, i, true, true, false);
	cur_r = get_rad(cent(i) - 1, until - 1);
	dp.apply_seg(i + 1, i + cur_r, dp, i - cur_r, i - 1, true, true, false);
}
...
void finalize_predict(int start, int until)
{
	...
	for (int _ = 0; _ < (int)active_per.size(); _++)
	{
		...
		if (lpos(right_baby) - get_rad(right_baby, until - 1) + 1 < left_side[per])
			continue;
		if (rpos(right_baby) + get_rad(right_baby, until - 1) - 1 < until - 1 &&
			lpos(right_baby) - get_rad(right_baby, until - 1) + 1 > left_side[per])
		{
			continue;
		}
		...
		for (int it = 0; it < (int)babies[per].size(); it++)
		{
			...
			while (i_from < i_until)
			{
				...
				per_dp[per].apply_seg(from, to, dp, dp_coef - (i_from + can - 1), dp_coef - i_from, false);
				i_from += can;
			}
		}
	}
